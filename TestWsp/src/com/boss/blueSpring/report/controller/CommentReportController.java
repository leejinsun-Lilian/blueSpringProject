...
//		System.out.println(memberNo);
		...
		try {			
			...
			int result = service.insertCommentReport(map);
			
			if(result > 0) { // DB 삽입 성공 시 result에는 삽입한 글 번호가 저장되어있다.
				int blindCheck = service.commentBlind(map);
				if(blindCheck > 0) {
					swalIcon = "success";
					swalTitle = "신고 접수가 완료되었습니다.";
					path = "WEB-INF/views/board/commentReport.jsp";
				} else {
					swalIcon = "error";
